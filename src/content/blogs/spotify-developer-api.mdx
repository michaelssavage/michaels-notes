...
On the projects page, you can see what I'm currently listening to and what my top tracks are on Spotify. It was an interesting project to implement, helped by this blog <Anchor link="https://navidanindya.info/writing/spotify-now-playing-nuxt/" variant="link" />. Setting up the API, retrieving data via Bruno requests, and then calling the Spotify API using fetch requests gave me good insight to OAuth.

1. Create an app on the Spotify developer dashboard

I first needed to visit the <Anchor link="https://developer.spotify.com/dashboard/" text="Spotify Developer Dashboard" variant="link" /> and log in with my Spotify credentials to create a new app. This generated a *Client ID* and *Client Secret* which I saved somewhere safe. I set the redirect URI to the local version of my site, which was `http:localhost:3000`.
...
The `client_id:client_secret` combination needs to be encoded in Base64 format. Use a site like <Anchor link="https://www.base64encode.org/." variant="link" /> to get the encoded string.
...
I used Bruno for sending the POST requests. I set the endpoint, the headers and the body as x-www-form-urlencoded fields.
...
The response from the endpoint has some fields but the important field I wanted was the `refresh_token`. This token doesn't expire, so it must be kept safe and not shared with anyone.

**Update (Jul 2026):** this used to be true, but Spotify now caps refresh tokens to a fixed lifetime — check the "Refresh Token Lifetime" field in your app's dashboard (currently 180 days). Once it expires, the refresh call starts failing with `invalid_grant: Refresh token revoked`, and you have to redo this whole authorization flow to mint a new one.
...
The important field this time is the `access_token` which is used to retrieve my data and expires in one hour from the request time. Below is the JSON response returned from Bruno.
...
Once I had everything working in Bruno, I could finally move my work over to my React site and show some data. Here is how the fetch request looked:
...
I will continue to improve this Spotify implementation and have added the following:

- ~~a music preview that you can play and pause~~ Not possible due to API limitations.
