...
On the blog page, you can see what I'm currently listening to and what my top tracks are on Spotify. It was a fun challenge to implement, helped by this blog <Anchor link="https://navidanindya.info/writing/spotify-now-playing-nuxt/." variant="link" isExternal /> Setting up the API, retrieving the desired data via Postman requests, and then calling the Spotify API using fetch requests took a bit of time. Here, I just go through briefly how I set it all up.

## Create an App on the Spotify Developer Dashboard

Visit the <Anchor link="https://developer.spotify.com/dashboard/" text="Spotify Developer Dashboard" variant="link" isExternal /> and log in with your Spotify credentials to create a new app.
...
The `client_id:client_secret` combination needs to be encoded in Base64 format. You can use a function or a site like <Anchor link="https://www.base64encode.org/." variant="link" isExternal />
...
I used Postman for sending the requests but here is the URL, Headers, and Body. You can also view the <Anchor link="https://navidanindya.info/writing/spotify-now-playing-nuxt/" text="linked blog" variant="link" isExternal /> for the `curl` request method.
