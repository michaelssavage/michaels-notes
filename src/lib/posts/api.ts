...
const AUTHORS_DIRECTORY_NAME = `_author`;
...
function filterByPublishedPostsOnly(post: BlogPost) {
  if (!process.env.production || !("live" in post)) return true;
  return post.live;
}

function sortBlogPostByDate(item: BlogPost, nextItem: BlogPost) {
  if (!item.date || !nextItem.date) return 1;
...
export function getPostsByCollection(collectionSlug: string) {
  ...
  return getAllPosts(
    (item) => item.collection?.name.toLowerCase() === collection.name.toLowerCase()
  );
}

export function getAllPosts(filterFn: (post: Partial<BlogPost>) => boolean = () => true) {
  const foundPosts = posts.map(getPostBySlug).filter(Boolean) as BlogPost[];
  return foundPosts.filter(filterByPublishedPostsOnly).filter(filterFn).sort(sortBlogPostByDate);
}
